...
export default function Home({ data }) {
  ...
  const handleSave = (item, index) => {
      setSaveClicked([...saveClicked, index])
      const previousMovies = JSON.parse(localStorage.getItem('savedStories')) || []
      previousMovies.push(item)
      localStorage.setItem('savedStories', JSON.stringify(previousMovies))
  }

  const checkIfSaveClicked = (index) => {
    if(saveClicked.includes(index)) {
        return true
    }
    return false;
  }
  ...
  return (
    <div>
      <div className="w-full max-h-screen-lg  items-center justify-center flex-col mx-auto">
        <div className="flex justify-center">
          <div className="flex-col space-y-7 text-4xl text-center absolute top-0 font-bold font-mono sm:text-5xl lg:text-8xl text-white mt-60">
            ...
            <button className="font-sans flex  text-white bg-blue-600 focus:outline-none hover:-translate-y-0.5 transform transition justify-center items-center mx-auto text-lg hover:bg-white hover:text-purple-700 py-3 px-4 rounded-full">
              Get Started
      ...
      <div className="flex justify-center align-center">
        <div className="text-6xl font-sans  text-center mt-24 mb-4   font-semibold tracking-tighter lg:text-7xl text-gradient title-font">
          Pictures of the day
        </div>
      </div>

      <div className="flex-col  flex items-center text-lg justify-items-center mx-auto p-6 rounded max-w-6xl">
        <p className="text-gray-500 max-w-3xl text-center mx-2 my-6 italic">
          "I want to know why the universe exist, why there is something greater
          than nothing."
        </p>
        <div className="flex items-center justify-center">
          <img
            src="/steve.jpg"
            className="rounded-full mr-2 h-16 w-16 object-cover"
          />
          <div>
            <p>Steven Hawking</p>
...
export const getServerSideProps = async () => {
  const res = await fetch(
    `https://api.nasa.gov/planetary/apod?api_key=${process.env.API_KEY}&count=3`
  );
